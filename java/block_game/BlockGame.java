...
public class BlockGame extends JPanel implements Runnable, MouseListener, MouseMotionListener, KeyListener, FocusListener{
    ...
    public BlockGame() {
		...
		JButton reflectMainDiagButton = new JButton("Reflect Main Diagonal \u2921");
		JButton reflectAntiDiagButton = new JButton("Reflect Anti-Diagonal \u2922");
		
		condenseButton.setFont(font);
		smashButton.setFont(font);
		rotateCWButton.setFont(font);
		rotateCCWButton.setFont(font);
		reflectMainDiagButton.setFont(font);
		reflectAntiDiagButton.setFont(font);

		condenseButton.setFocusable(false);
		smashButton.setFocusable(false);
		rotateCWButton.setFocusable(false);
		rotateCCWButton.setFocusable(false);
		reflectMainDiagButton.setFocusable(false);
		reflectAntiDiagButton.setFocusable(false);

		condenseButton.setAlignmentX(Component.CENTER_ALIGNMENT);
		...
		reflectMainDiagButton.setAlignmentX(Component.CENTER_ALIGNMENT);
		reflectAntiDiagButton.setAlignmentX(Component.CENTER_ALIGNMENT);

		side.add(condenseButton);
		side.add(smashButton);
		side.add(rotateCWButton);
		side.add(rotateCCWButton);
		side.add(reflectMainDiagButton);
		side.add(reflectAntiDiagButton);
		...
		reflectMainDiagButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (selected != null) {
					selected.reflect(0);
		...
		reflectAntiDiagButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (selected != null) {
					selected.reflect(1);
		...
		String[] goalSelect = new String[]{"Blob", "Symmetry"};
		...
		if (p1GoalType < 0.5) {
			this.p1goal = new SymmetryGoal(gen.nextInt(2));
		} else {
		...
		if (playerCount == 1) {
			...
			if (p2GoalType < 0.5) {
				this.p2goal = new SymmetryGoal(gen.nextInt(2));
			} else {
	...
	private void updateScores() {
		...
		p1ScoreLabel.setText("Player 1's " + (p1goal instanceof BlobGoal ? "area" : "symmetry ") + " score: " + this.p1Score);
		p2ScoreLabel.setText("Player 2's " + (p2goal instanceof BlobGoal ? "area" : "symmetry ") + " score: " + this.p2Score);
	}
